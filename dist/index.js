...
import { CreateRepositorySchema, CreateIssueSchema, ListIssuesSchema, CreateCommitSchema, } from './schemas/index.js';
import { createRepository, createIssue, listIssues, createCommit, listRepositories, } from './github/operations.js';
import { handleGitHubError, getErrorMessage } from './errors/handler.js';
const server = new McpServer({
    name: 'github-mcp-server',
    version: '1.0.0',
});
server.tool('list-repositories', 'Lista los repositorios del usuario autenticado', {}, async () => {
    try {
        const repos = await listRepositories();
        const reposList = repos
            .map((repo) => `- ${repo.name}: ${repo.description || 'Sin descripción'} (${repo.private ? 'Privado' : 'Público'})`)
            .join('\n');
        return {
            content: [{ type: 'text', text: `Tus repositorios:\n${reposList}` }],
        };
    }
    catch (error) {
        const appError = handleGitHubError(error);
        return {
            content: [{ type: 'text', text: getErrorMessage(appError) }],
            isError: true,
        };
    }
});
server.tool('create-repository', 'Crea un nuevo repositorio en GitHub', {
...
server.tool('create-issue', 'Abre un issue en un repositorio', {
...
server.tool('list-issues', 'Lista issues de un repositorio', {
...
server.tool('create-commit', 'Crea un commit con cambios en un archivo', {
